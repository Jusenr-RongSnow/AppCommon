...
import com.myself.appcommon.http.command.GetImageRequest;
import com.myself.appcommon.http.command.GetImageResponse;
import com.myself.appcommon.http.model.HttpResult;
import com.myself.appcommon.http.model.ImageInfo;
import com.myself.appcommon.http.model.ImageResult;
...
import com.myself.appcommon.util.ImageUtil;
...
public class PngImageDownloadPool {
    ...
    public void addTask(ImageType imageType, GetImageRequest request,
                        GetImageResponse response) {
        synchronized (mImagesPrepareQueue) {
            ImageInfo imageinfo = new ImageInfo(imageType, request, response);

            if (mImagesPrepareQueue.contains(imageinfo)
                    || (mImageProcessingArray[0] != null && mImageProcessingArray[0]
                    .equals(imageinfo))
                    || (mImageProcessingArray[1] != null && mImageProcessingArray[1]
                    .equals(imageinfo))) {
                return;
            }
            ...
            if (length >= MAX_PREPARE_NUM) {
                ...
                onRecvError(removeinfo.getResponse(),
                        removeinfo.getImageType(), removeinfo.getRequest()
                                .getTag(), ImageResult.ERROR_NET_ACCESS);
            }
    ...
    private void onRecvError(final GetImageResponse response,
                             final ImageType imageType, final Object tag, final int errorCode) {
    ...
    private void onRecvOK(final GetImageResponse response,
                          final ImageType imageType, final Object tag, final Bitmap bitmap,
                          final String path) {
        BaseApplication.getInstance().runOnUIThread(new Runnable() {
    ...
    private class ImageThread extends Thread {
        ...
        public Bitmap getBitmapFromNet(GetImageRequest request,
                                       GetImageResponse response) {
            ...
            if (result.getResultCode() == HttpEngine.HttpCode.STATUS_OK
        ...
        public void run() {
            ...
            while (!cancel) {
                ...
                GetImageRequest request = mImageProcessingArray[runId]
                        .getRequest();
                GetImageResponse response = mImageProcessingArray[runId]
                        .getResponse();
                ...
                if (NetStatusReceiver.netStatus == NetStatusReceiver.NETSTATUS_INAVAILABLE) {
                    onRecvError(response, type, request.getTag(),
                            ImageResult.ERROR_URL_NULL);
                } else {
                    try {
                        ...
                        if (returnBitmap == null) {
                            onRecvError(response, type, request.getTag(),
                                    ImageResult.ERROR_NET_ACCESS);
                        } else {
                            if (ExternalStorageReceiver.isSDCardMounted) {
                                ImageUtil.saveBitmapPNG(returnBitmap,
                                        request.getFilePath(),
                                        ImageUtil.QUALITY_HIGH);
                            }
                            if (returnBitmap != null) {
                                TaskManager.putImageInCache(type,
                                        request.getFilePath(), returnBitmap);
                                onRecvOK(response, type, request.getTag(),
                                        returnBitmap, request.getFilePath());
                            } else {
                                onRecvError(response, type, request.getTag(),
                                        ImageResult.ERROR_OOM);
                            }
                        }
